...
class Str_type{
    ...
    public:
        ...
        Str_type operator=(const Str_type &obj); //Присваивание cтроки объекту Str_type
        ...
        friend ostream &operator<<(ostream &stream, const Str_type &obj);
...
Str_type Str_type::operator=(const Str_type &obj)
{
    Str_type tmp;
...
ostream &operator<<(ostream &stream, const Str_type &obj)
{
    stream<<obj.p;
    return stream;
}
...
void sort_string(Str_type *data) //cортировка по алфавиту
{
    Str_type tmp;
    ...
    for (int i=0; i<N-1; i++)
        ...
        {
            if (strcmp(data[j].p,data[j+1].p)>0)
            {
                tmp=data[j];
                data[j]=data[j+1];
                data[j+1]=tmp;
...
int main()
{
    ...
    //Массив объектов
    Str_type data[3]= {Str_type("сортировка"), Str_type("строк"), Str_type("массива")};
